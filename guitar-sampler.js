(function initFreezaGuitarSampler(global) {
  ...
  function decode(ctx, data) {
    return new Promise((resolve, reject) => {
      let settled = false;
      const timer = setTimeout(() => fail(new Error('guitar sample decode timeout')), 15000);
      const done = value => {
        if (!settled) {
          settled = true;
          clearTimeout(timer);
  ...
  function fetchSample(url, timeoutMs = 15000) {
  ...
  function nearestLoadedRegion(manifest, midi, velocity) {
    const key = Math.round(Number(midi));
    const vel = Math.max(0, Math.min(127, Math.round(Number(velocity || 0) * 127)));
    const loaded = manifest.regions.filter(region => buffers.has(bufferKey(manifest, region)));
    if (!loaded.length) return null;
    const distance = region => Math.abs(region.center - key)
      + (vel < region.lovel ? region.lovel - vel : vel > region.hivel ? vel - region.hivel : 0) / 12;
    return loaded.reduce((best, region) => distance(region) < distance(best) ? region : best);
  }
  ...
  function preloadStartup(ctx, code, onProgress = null) {
    const manifest = manifestForCode(code);
    if (!manifest) return Promise.resolve([]);
    // A soft and a strong sample nearest middle C are enough to keep the whole
    // song electric-guitar-only: playbackRate covers the remaining notes while
    // their exact regions continue loading on demand in the background.
    const velocityBands = [...new Map(manifest.regions.map(region => [
      `${region.lovel}:${region.hivel}`,
      region,
    ])).keys()];
    const essential = velocityBands.map(band => {
      const [lovel, hivel] = band.split(':').map(Number);
      return manifest.regions
        .filter(region => region.lovel === lovel && region.hivel === hivel)
        .reduce((best, region) => Math.abs(region.center - 60) < Math.abs(best.center - 60) ? region : best);
    });
    let settled = 0;
    const total = essential.length;
    if (typeof onProgress === 'function') onProgress(0, total);
    return Promise.allSettled(essential.map(region => loadRegion(ctx, manifest, region).finally(() => {
      settled++;
      if (typeof onProgress === 'function') onProgress(settled, total);
    })));
  }
  ...
  function play(ctx, destination, code, midi, duration, velocity, gainScale, when = ctx.currentTime) {
    ...
    const preferredRegion = chooseRegion(manifest, midi, velocity);
    let region = preferredRegion;
    let buffer = buffers.get(bufferKey(manifest, region));
    if (!buffer) {
      // Never change timbre while an exact sample is arriving.  Re-pitch an
      // already decoded region from the same electric-guitar library instead.
      region = nearestLoadedRegion(manifest, midi, velocity);
      buffer = region ? buffers.get(bufferKey(manifest, region)) : null;
      loadRegion(ctx, manifest, preferredRegion).catch(error => console.warn('Guitar sample load failed:', error));
  ...
  global.FreezaGuitarSampler = Object.freeze({ libraryForCode, preload, preloadAll, preloadStartup, play });
})(window);
