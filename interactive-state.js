(function initInteractiveState(global) {
  ...
  function judgeOneKeyWaitingTiming(progress) {
    const judged = judgeOneKeyTiming(progress, false);
    if (judged.accepted || !Number.isFinite(Number(progress)) || Number(progress) <= 120) return judged;
    // 小节已经自然结束后，播放器必须允许迟到输入继续下一小节；
    // 评分仍为 MISS，但不能让 120% 之后永久进入“按什么都无效”的死锁。
    return Object.freeze({
      accepted: true,
